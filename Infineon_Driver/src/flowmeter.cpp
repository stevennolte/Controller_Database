...
void FlowMeter::loop(){
    ...
    if (cnt > 0){
        uint64_t timeStamp = esp_timer_get_time()- previousTimestamp;
        previousTimestamp = esp_timer_get_time();
        uint16_t _cnt = cnt;
        cnt = 0;
        frequency = frequency*frequencyFilter + ((float)(_cnt*1000000))/((float)(timeStamp))*(1.0-frequencyFilter);
        // frequency = ((float)(cnt*1000000))/((float)(esp_timer_get_time() - previousTimestamp));
